...
void AntColonySystem::InitParameter(vector<CPart> testData, double allDistance[partNum][partNum])
{
    ...
    for (int i = 0; i < partNum; i++)
    {
        for (int j = 0; j < partNum; j++)
        {
            ...
            if (i != j)
            {
                visible[i][j] = 1/double(testData[j].getDeadline()); //这里有使用allDistance初始化
            }
        }
    }
}
...
//全局信息素更新
void AntColonySystem::UpdateGlobalPathRule(tourType bestTour, double gloalbestValue) //fix:此函数要改，入炉序列生成后根据时间调整
{
    for(int i=0;i<partNum;i++)
    {
        for(int j=0;j<partNum;j++)
    ...
    for (int i = 0; i < partNum; i++)
    {
        ...
        info[row][col] += rou * 1/double(gloalbestValue); //只给最优路径中节点增加
        //一个时间相反的数，分数越高说明时间越小
    }
}
